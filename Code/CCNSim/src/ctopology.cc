//=========================================================================
//  cTopology_s.CC - part of
//
//                  OMNeT++/OMNEST
//           Discrete System Simulation in C++
//
//   Member functions of
//     cTopology_s : network topology to find shortest paths etc.
//
//  Author: Andras Varga
//
//=========================================================================
...
Register_Class(cTopology_s);


cTopology_s::LinkIn *cTopology_s::Node::getLinkIn(int i)
{
    if (i<0 || i>=num_in_links)
        throw cRuntimeError("cTopology_s::Node::getLinkIn: invalid link index %d", i);
    return (cTopology_s::LinkIn *)in_links[i];
}

cTopology_s::LinkOut *cTopology_s::Node::getLinkOut(int i)
{
    if (i<0 || i>=num_out_links)
        throw cRuntimeError("cTopology_s::Node::getLinkOut: invalid index %d", i);
    return (cTopology_s::LinkOut *)(out_links+i);
}

//----

cTopology_s::cTopology_s(const char *name) : cOwnedObject(name)
{
    num_nodes = 0;
    nodev = NULL;
}

cTopology_s::cTopology_s(const cTopology_s& topo) : cOwnedObject()
{
    nodev = NULL;
    setName(topo.getName());
    cTopology_s::operator=(topo);
}

cTopology_s::~cTopology_s()
{
    clear();
}

std::string cTopology_s::info() const
{
...
void cTopology_s::parsimPack(cCommBuffer *buffer)
{
    throw cRuntimeError(this,"parsimPack() not implemented");
}

void cTopology_s::parsimUnpack(cCommBuffer *buffer)
{
    throw cRuntimeError(this,"parsimUnpack() not implemented");
}

cTopology_s& cTopology_s::operator=(const cTopology_s&)
{
    throw cRuntimeError(this,"operator= not implemented yet");
}

void cTopology_s::clear()
{
...
void cTopology_s::extractByModulePath(const std::vector<std::string>& fullPathPatterns)
{
    extractFromNetwork(selectByModulePath, (void *)&fullPathPatterns);
}

void cTopology_s::extractByNedTypeName(const std::vector<std::string>& nedTypeNames)
{
    extractFromNetwork(selectByNedTypeName, (void *)&nedTypeNames);
}

void cTopology_s::extractByProperty(const char *propertyName, const char *value)
{
    struct {const char *name; const char *value;} data = {propertyName, value};
    extractFromNetwork(selectByProperty, (void *)&data);
}

void cTopology_s::extractByParameter(const char *paramName, const char *paramValue)
{
...
{
    cTopology_s::Predicate *predicate = (cTopology_s::Predicate *)data;
    return predicate->matches(mod);
}

void cTopology_s::extractFromNetwork(Predicate *predicate)
{
    extractFromNetwork(selectByPredicate, (void *)predicate);
}

void cTopology_s::extractFromNetwork(bool (*selfunc)(cModule *,void *), void *data)
{
    clear();
    ...
    for (int mod_id=0; mod_id<=simulation.getLastModuleId(); mod_id++)
    {
        ...
        if (mod && selfunc(mod,data))
        {
            ...
            temp_nodev[k].in_links = new cTopology_s::Link *[mod->gateCount()];
    ...
    for (int k=0; k<num_nodes; k++)
    {
        ...
        cTopology_s::Link *temp_out_links = new cTopology_s::Link[mod->gateCount()];
        ...
        nodev[k].out_links = new cTopology_s::Link[n_out];
        memcpy(nodev[k].out_links, temp_out_links, n_out*sizeof(cTopology_s::Link));
    ...
    for (int k=0; k<num_nodes; k++)
    {
        ...
        {
            cTopology_s::Link *link = &nodev[k].out_links[l];
            link->dest_node->in_links[link->dest_node->num_in_links++] = link;
        }
    }
}

cTopology_s::Node *cTopology_s::getNode(int i)
{
    if (i<0 || i>=num_nodes)
...
cTopology_s::Node *cTopology_s::getNodeFor(cModule *mod)
{
...
void cTopology_s::calculateUnweightedSingleShortestPathsTo(Node *_target)
{
...
void cTopology_s::weightedSingleShortestPathsTo(Node *_target)
{
    // multiple paths not supported :-(
...
void cTopology_s::weightedMultiShortestPathsTo(Node *_target)
{
...
void cTopology_s::extractWeight(std::string file){
using namespace std;
     ifstream fi;
     char row[100];
     char *path;
	 ...
	 //cout<<"Numero link "<<source->num_out_links<<endl;
		...
		if(d==((try_dest->getModule())->getIndex())){
			cTopology_s::Link * link=source->getLinkOut(i);
 ...
 * the cTopology_s class. The algorithm has been published on the paper "A faster
 * Algorithm for Betweenness Centrality by Ulrik Brandes and has a complexity
 * of O(nm)
 */
void cTopology_s::betweenness_centrality(){
    std::deque<Node*> S,Q;
